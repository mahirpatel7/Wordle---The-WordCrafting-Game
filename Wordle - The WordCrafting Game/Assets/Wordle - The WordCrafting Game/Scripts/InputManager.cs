...
public class InputManager : MonoBehaviour
{
    ...
    public void CheckWord()
    {
        ...
        if (wordToCheck == secretWord)
        {
            SetLevelComplete();
            DisableTryButton();
            // Disable further input since the game is complete
            canAddLetter = false;
        }
        else
        {
           // Debug.Log("Wrong word");
            ...
            // Allow letters to be added to the next word container
            currentWordContainerIndex++;

            // Check if there are more word containers
            if (currentWordContainerIndex < wordContainers.Length)
            {
                canAddLetter = true; // Re-enable adding letters
                DisableTryButton();
            }
            else
            {
